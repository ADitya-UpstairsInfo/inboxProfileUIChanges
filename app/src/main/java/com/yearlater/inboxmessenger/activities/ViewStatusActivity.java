...
import static com.yearlater.inboxmessenger.activities.main.messaging.ChatActivity.CAMERA_REQUEST;
import static com.yearlater.inboxmessenger.activities.main.messaging.ChatActivity.MAX_FILE_SIZE;
import static com.yearlater.inboxmessenger.activities.main.messaging.ChatActivity.PICK_CONTACT_REQUEST;
import static com.yearlater.inboxmessenger.activities.main.messaging.ChatActivity.PICK_GALLERY_REQUEST;
import static com.yearlater.inboxmessenger.activities.main.messaging.ChatActivity.PICK_LOCATION_REQUEST;
import static com.yearlater.inboxmessenger.activities.main.messaging.ChatActivity.PICK_MUSIC_REQUEST;
import static com.yearlater.inboxmessenger.activities.main.messaging.ChatActivity.PICK_NUMBERS_FOR_CONTACT_REQUEST;
import static com.yearlater.inboxmessenger.activities.main.messaging.ChatActivity.RECORD_START_AUDIO_LENGTH;
import static com.yearlater.inboxmessenger.utils.FontUtil.isFontExists;
...
import com.yearlater.inboxmessenger.adapters.StatusSeenByAdapter;
import com.yearlater.inboxmessenger.adapters.StatusSeenByCallback;
...
import com.yearlater.inboxmessenger.utils.KeyboardUtils;
...
import com.yearlater.inboxmessenger.utils.StatusHelper;
...
import com.yearlater.inboxmessenger.utils.network.StatusManager;
...
//        quotedMessageFrame.setVisibility(View.VISIBLE);
        ...
        llviewCLick.setOnClickListener(view -> {
            if (isSeenByExpanded()) {
                seenByBottomsheetBehavior.setState(BottomSheetBehavior.STATE_COLLAPSED);
                arrowUp.animate().rotation(0).setDuration(200).start();
                rvSeenBy.animate().translationY(0).start();
                resume();

            } else {
                seenByBottomsheetBehavior.setState(BottomSheetBehavior.STATE_EXPANDED);
        ...
        llReply.setOnClickListener(view -> {
            if (isReplyExpanded()) {
                replyBehavior.setState(BottomSheetBehavior.STATE_COLLAPSED);
                KeyboardHelper.hideSoftKeyboard(ViewStatusActivity.this, replyArrowUp);
                replyArrowUp.animate().rotation(0).setDuration(200).start();
                replyDimView.setVisibility(View.GONE);
                resume();

            } else {
                replyBehavior.setState(BottomSheetBehavior.STATE_EXPANDED);
    ...
    private void setupAdapter(Status status) {
        ...
        Observable<Pair<String, List<StatusSeenBy>>> map = statusManager.getStatusSeenByList(status.getStatusId());

        map.subscribe(pair ->{
            Log.e("listhrhhe" , " " +pair.component1());
        });
...
//        quotedMessageFrame.setBackgroundColor(ContextCompat.getColor(this, R.color.quoted_sent_background_color));
    ...
    private void initViews() {
        ...
        llviewCLick = findViewById(R.id.llviewCLick);
...
//        quotedMessageFrame = findViewById(R.id.quoted_message_frame);
    ...
    private void loadStatus(final Status status) {
        ...
        tvSeenCount.setText(statusSeenBy.size() + "");
        Log.e("seen+ 1132", "" +statusSeenBy.size());
        setupAdapter(status);
        ...
        if (!status.getUserId().equals(FireManager.getUid()) && !status.isSeenCountSent()) {
            getDisposables().add(statusManager.setStatusSeen(userId, status.getStatusId()).subscribe(() -> {
                FireConstants.statusCountRef.child(status.getUserId()).child(status.getStatusId()).setValue((statusSeenBy.size()+1));
            }, throwable -> {
