...
import com.yearlater.inboxmessenger.adapters.NewGroupAdapter
import com.yearlater.inboxmessenger.adapters.NewGroupSelectedUsersAdapter
    ...
    private fun createGroup(groupTitle: String) {
        ...
        if (NetworkHelper.isConnected(this)) {
            groupManager.createNewGroup(groupTitle, selectedUsers).subscribe { groupUser, throwable ->
                ...
                if (throwable != null) {
                    Toast.makeText(this@NewGroupActivity,throwable.message, Toast.LENGTH_SHORT).show()
                } else {
                    val intent = Intent(this@NewGroupActivity, ChatActivity::class.java)
                    intent.putExtra(IntentUtils.UID, groupUser.uid)
                    Log.e("group id",groupUser.uid)
                    startActivity(intent)
                    finish()
                }
            }.addTo(disposables)
