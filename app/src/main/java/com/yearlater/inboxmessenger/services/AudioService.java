...
import com.yearlater.inboxmessenger.model.constants.HeadsetState;
import com.yearlater.inboxmessenger.model.constants.SensorState;
import com.yearlater.inboxmessenger.utils.AudioHelper;
...
import com.yearlater.inboxmessenger.utils.ProximitySensor;
...
public class AudioService extends Service implements AudioManager.OnAudioFocusChangeListener, ProximitySensor.Delegate {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (intent != null && intent.getAction() != null) {
            ...
            } else if(action.equals(IntentUtils.ACTION_SPEED_UP)) {
                Log.e("intent working","ok");
                float playbacks = intent.getFloatExtra(IntentUtils.PLAYBACK_SPEED,1);
                speedup(playbacks);
            }
...
//
    ...
    private int speedup(float playbacks) {


        if(isPlaying()) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                Log.e("Playing using this"," ok");
                mediaPlayer.setPlaybackParams(mediaPlayer.getPlaybackParams().setSpeed(playbacks));
            }

        }
        Log.e("playback audioservice",playbacks +"");
        return (int) playbacks;
